...
class user { //controls the user account
  public $conf,
    $error;

  function __construct ($conf) { //grab conf and set error to null
    $this ->conf = $conf;
    $this ->error = 0;
  }
  public function login ($name, $pass, $remember) { //login function
  ...
  public function signup ($name, $pass, $pass1, $file) { //sign up function
    $name = htmlspecialchars ($name); // getting user inputs
    $pass = htmlspecialchars ($pass);
    ...
    if ($name && $pass) {
      $check = $this ->conf ['database'] ->select ('name',
        'users',
        'name',
        $name
      );
  ...
  public function edit ($dir, $currentN, $pfp, $desc, $userName, $pass, $confPass) {
    if ($userName) {
      if (strlen ($userName) <= 15) {
        $currentN = $userName;
        $this ->conf ['database'] ->update ('users',
          'name',
          $userName,
          'id',
          $_SESSION ['id']
